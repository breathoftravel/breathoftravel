...
async function getBestAdventures() {
    const res = await fetch('https://sukhantharot.github.io/dummy-fake-json/best-adventure.json');
    return res.json();
}
        ...
        <>
            <div className="flex flex-col w-full py-14 md:p-14 items-center justify-center">
                ...
                <div className={`flex flex-wrap md:flex-nowrap gap-6 py-4 justify-center md:justify-between`}>
                    {bestIslands?.map((bestIsland, index) => {
                        const jsonLd = {
            ...
            <div className="flex flex-col w-full py-14 md:p-14 items-center justify-center">
                ...
                <div className={`flex flex-wrap md:flex-nowrap gap-6 py-4 justify-center md:justify-between`}>
                    {bestAdventures?.map((adventure, index) => {
                        const jsonLd = {
                            '@context': 'https://schema.org',
                            '@type': 'Product',
                        ...
                        return (
                            <div key={adventure.id} className={`w-5/12 md:w-1/5 lg:w-1/4`}>
                                <Link href={`/product/${adventure.id}`}>
                                    <script
                                        type="application/ld+json"
                                        dangerouslySetInnerHTML={{__html: JSON.stringify(jsonLd)}}
                                    />
